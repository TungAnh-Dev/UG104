...
namespace CasualGameArchitecture.Scripts.UserData
{
    ...
    {
        private readonly Dictionary<Type, ILocalData> dataCache = new Dictionary<Type, ILocalData>();
        ...
        public T GetData<T>() where T : class
        {
            ...
            if (this.dataCache.TryGetValue(type, out var cachedData))
            {
                return (T)cachedData;
            }
        ...
        public void SaveAll()
        {
            ...
            foreach (var data in this.dataCache.Values)
            {
                SaveInternal(data);
            }
            PlayerPrefs.Save();
    ...
    #if UNITY_EDITOR
        public Dictionary<Type, ILocalData> GetRawCache() => this.dataCache;
    #endif
    }
}
